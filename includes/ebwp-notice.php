...
class Fascinate_Ebwp_Notice {
	...
	const META_EXPIRE_AFTER = 'fascinate_ebwp_notice_expire_after';

	const META_ACTION_TYPE = 'fascinate_ebwp_notice_expire_after';

	const KEY_SUBMIT = 'fascinate_ebwp_notice';
	...
	public function notice_styles() {
		...
		<style>
			...
			#fascinate-ebwp-notice .message.et-notice-message {
				width: 100%;
				padding: 10px;
			}

			#fascinate-ebwp-notice .message.et-notice-message img {
				width: 53px;
				float: left;
				margin-right: 10px;
			}

			#fascinate-ebwp-notice .et-notice-message h1{
				margin: 0; padding: 0;
				font-size: 24px;
				font-weight: 400;
			}
			#fascinate-ebwp-notice .et-notice-message p {
				font-size: 14px;
				margin: 0;
			}
			#fascinate-ebwp-notice .et-notice-message p strong {
				color: #2271B1;
			}
			.et-notice-actions {
				width: 100%;
			...
			#fascinate-ebwp-notice .et-notice-actions button, #fascinate-ebwp-notice .et-notice-actions a.button-primary{
	...
	public function admin_notice() {
		...
		if ( $this->install_activate ) {
			?>
			<div id="fascinate-ebwp-notice" class="notice is-dismissible">
				<div class="message et-notice-message">

					<img class="logo-icon" src="<?php echo esc_url( self::LOGO_URL ); ?>">

					<h1><?php esc_html_e( 'Thank You !!!', 'fascinate' ); ?></h1>
					...
					$string = sprintf( __( '%s has been installed and activated successfully.', 'fascinate' ), $plugin_link );
		 ...
		 */
		?>
		<div id="fascinate-ebwp-notice" class="notice">
			<div class="message et-notice-message">

				<img class="logo-icon" src="<?php echo esc_url( self::LOGO_URL ); ?>">

				<h1><?php esc_html_e( 'Your website is super precious !', 'fascinate' ); ?></h1>
				<?php
				...
				if ( 'not-installed' === $status ) {
					/* translators: %s is the Everest Backup plugin name wrapped with html. */
					$string = sprintf( __( 'The best way to protect your website is by using %s plugin to keep your data safe and secure to remote storage..', 'fascinate' ), $plugin_link );
				} else {
					/* translators: %s is the Everest Backup plugin name wrapped with html. */
					$string = sprintf( __( 'You are just one step away. Please activate %s plugin and backup your website to keep your data safe and secure.', 'fascinate' ), $plugin_link );
				}
			...
			<form method="post">
				...
				if ( 'not-installed' === $status ) {
					?>
					<button name="install" value="1" class="button-primary"><?php esc_html_e( 'Install & Activate', 'fascinate' ); ?></button>
					<button name="remind" value="1" class="button"><?php esc_html_e( 'Remind me later', 'fascinate' ); ?></button>
					<button name="never" value="1" class="button-link"><?php esc_html_e( "Don't show this again", 'fascinate' ); ?></button>
					<?php
				} else {
					$activation_link = $this->get_plugin_activation_link();

					?>
					<a href="<?php echo esc_url( $activation_link ); ?>" class="button-primary"><?php esc_html_e( 'Activate Plugin', 'fascinate' ); ?></a>
					<?php
				}
				?>
			</form>
