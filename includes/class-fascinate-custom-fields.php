...
if ( ! class_exists( 'Fascinate_Custom_Fields' ) ) {
	/**
	 * Class to register, render, and save post meta for sidebar position and single layout.
	 *
	 * @since 1.0.0
	 *
	 * @package Fascinate
	...
	class Fascinate_Custom_Fields {
		...
		public function register_post_meta() {
			...
			add_meta_box(
				'single_layout_metabox',
				esc_html__( 'Layout', 'fascinate' ),
				array( $this, 'single_layout_meta' ),
				array( 'post', 'page' ),
				'side',
				'default'
			);
		...
		public function save_sidebar_position_meta() {
			...
			if (
				! isset( $_POST['sidebar_position_meta_nonce_id'] ) ||
				! wp_verify_nonce(
					sanitize_key( $_POST['sidebar_position_meta_nonce_id'] ),
					'sidebar_position_meta_nonce'
				)
			) {
				return;
			}
		...
		public function save_single_layout_meta() {
			...
			// If our nonce isn't there, or we can't verify it, bail.
			if (
				! isset( $_POST['single_layout_meta_nonce_id'] ) ||
				! wp_verify_nonce(
					sanitize_key( $_POST['single_layout_meta_nonce_id'] ),
					'single_layout_meta_nonce'
				)
			) {
				return;
			}
			...
			if ( isset( $_POST['single_layout'] ) ) {
				update_post_meta(
					$post->ID,
					'fascinate_single_layout',
					sanitize_text_field( wp_unslash( $_POST['single_layout'] ) )
				);
			}
		}
	}
